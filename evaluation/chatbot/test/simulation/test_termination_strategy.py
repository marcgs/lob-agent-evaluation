...
class TestLLMTerminationStrategy:
    ...
    async def test_support_ticket_creation_completed(
        self, support_ticket_strategy: LLMTerminationStrategy
    ):
        ...
        conversation_pairs = [
            ...
            (
                "assistant",
                "I'll help you create a support ticket. Please provide the details.",
            ),
            ("user", "My email client crashes on startup after the latest update."),
            (
                "assistant",
                "The support ticket has been successfully created! Here are the details:\n\n- **Ticket ID**: TKT-ABC123\n- **Status**: Created\n- **Created At**: 2025-10-09T10:00:00",
            ),
        ]
        ...
        assert result is True, (
            "Should terminate when support ticket is successfully created"
        )
    ...
    async def test_support_ticket_creation_in_progress(
        ...
        conversation_pairs = [
            ...
            (
                "assistant",
                "I'll help you create a support ticket. Please provide the following information:\n1. Title\n2. Description\n3. Priority",
            ),
            ("user", "Title: Email client issues"),
            (
                "assistant",
                "Thank you. Now please provide a detailed description of the issue.",
            ),
        ]
        ...
        assert result is False, (
            "Should not terminate when support ticket creation is still in progress"
        )

    @pytest.mark.asyncio
    async def test_different_task_completion_condition(
        self, generic_task_strategy: LLMTerminationStrategy
    ):
        ...
        conversation_pairs = [
            ...
            (
                "assistant",
                "I'll help you set up your email account. Let me guide you through the steps.",
            ),
            ("user", "Great, I'm ready."),
            (
                "assistant",
                "Perfect! Your email account has been successfully configured and is ready to use. You should now be able to send and receive emails.",
            ),
        ]
        ...
        assert result is True, (
            "Should terminate when the user's request has been completed"
        )
    ...
    async def test_maximum_iterations_termination(
        ...
        # Simple conversation
        conversation_pairs = [("user", "Hello"), ("assistant", "Hi there!")]

        messages = self._create_messages(conversation_pairs)

        # Manually set iteration count to maximum
        support_ticket_strategy.iteration_count = (
            support_ticket_strategy.maximum_iterations
        )
    ...
    @pytest.mark.asyncio
    async def test_minimal_conversation_history(
        self, support_ticket_strategy: LLMTerminationStrategy
    ):
        """Test behavior with minimal conversation history."""
        conversation_pairs = [("user", "Hello")]
    ...
    async def test_alternative_completion_phrases(
        ...
        conversation_pairs = [
            ...
            (
                "assistant",
                "Your support ticket has been created successfully with ID: TKT-XYZ789. The ticket is now in our system and assigned to the technical team.",
            ),
        ]
        ...
        assert result is True, (
            "Should recognize alternative phrasing for ticket creation confirmation"
        )
    ...
    async def test_false_positive_prevention(
        ...
        conversation_pairs = [
            ...
            (
                "assistant",
                "I understand you want to create a support ticket. Let me explain the process first.",
            ),
            ("user", "Okay, what do I need to provide?"),
            (
                "assistant",
                "To create a support ticket, you'll need to provide a title, description, and priority level. Once we have this information, the ticket will be created in our system.",
            ),
        ]
        ...
        assert result is False, (
            "Should not terminate when only discussing ticket creation process"
        )
